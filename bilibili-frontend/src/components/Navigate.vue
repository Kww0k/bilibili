  ...
  <transition name="fade">
    <div :class="[type === 'fly' ? 'fly' : type === 'simple' ? 'simple' : 'top']" style="height: 64px; width: 100%;">
      <ul class="left-entry">
        ...
        <li :class="{'left-title': true, 'jump': activeIndex === 0}" @mouseover="startJump(0)" @animationend="endJump(0)">
          番剧
        </li>
        <li :class="{'left-title': true, 'jump': activeIndex === 1}" @mouseover="startJump(1)" @animationend="endJump(1)">
          直播
        </li>
        <li :class="{'left-title': true, 'jump': activeIndex === 2}" @mouseover="startJump(2)" @animationend="endJump(2)">
          游戏中心
        </li>
        <li :class="{'left-title': true, 'jump': activeIndex === 3}" @mouseover="startJump(3)" @animationend="endJump(3)">
          会员购
        </li>
        <li :class="{'left-title': true, 'jump': activeIndex === 4}" @mouseover="startJump(4)" @animationend="endJump(4)">
          漫画
        </li>
        <li :class="{'left-title': true, 'jump': activeIndex === 5}" @mouseover="startJump(5)" @animationend="endJump(5)">
          赛事
        </li>
        <li class="left-title">
          ...
          <span style="margin-left: 5px">下载客户端</span>
        </li>
      </ul>
